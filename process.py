...
class Process:
    ...
    def get_stage(self, segments):
        ...
        for idx in result_stenosis.keys():
            ...
            # 用
            sis[idx] = len([segments[idx][i].get('斑块类型', '') for i in range(len(segments[idx])) if segments[idx][i].get('斑块类型', '') not in ['无', '']])
            ...
            elif any([segments[idx][i].get('斑块类型', '') not in ['无', ''] for i in range(len(segments[idx]))]) and all([value == 0 for value in result_stenosis[idx].values()]):
                stage_cadras[idx] = '1'
                print(f'{idx} is CADRADS 1')
        ...
        # 根据sis计算p分级：sis小于等于2为p1，sis3-4为p2，sis5-7为p3，sis大于等于8为p4
        stage_p = {}
        for idx in sis.keys():
            if sis[idx] <= 2:
                stage_p[idx] = 'P1'
        ...
        return stage_cadras, sis, stage_p

    # 写一个函数提取心肌桥信息
    def extract_myocardial_bridge(self, response):
        ...
        for idx in segments.keys():
            ...
            bridge = [segments[idx][i].get('是否存在心肌桥', '') for i in range(len(segments[idx]))]
            myocardial_bridge[idx] = all([segments[idx][i].get('是否存在心肌桥', '') in ['有', '是'] for i in range(len(segments[idx]))])
        # print多少心肌桥阳性
    ...
    def main(self, file):
        ...
        stage_cadras, sis, stage_p = self.get_stage(segments)
        result = {'coronary_artery_dominance': coronary_artery_dominance, 'coronary_artery_abnormal': coronary_artery_abnormal, 'stage_cadras': stage_cadras, 'sis': sis, 'stage_p': stage_p}
        # to pd.DataFrame
